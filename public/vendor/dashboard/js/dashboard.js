...
jQuery(document).ready(function ($) {
  ...
  $('body').on('click', '.js-fill-other-locales', function(e){
      e.preventDefault()
      let self = $(this)
      let input = self.closest('label').next()

      if ($.trim(input.val()) !== '') {
          Object.entries(locales).forEach(([key, value]) => {
              $('[name="' + key + '['+self.attr('data-name')+']"]').val(input.val())
          })
  ...
  $('body').on('click', '.js-clean-report', function(e){
    e.preventDefault()
